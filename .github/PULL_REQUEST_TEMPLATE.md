...
<!-- I keep LITE intentionally minimal at seven components. I need to hear why this belongs
...
I use Codex as the sole bot reviewer (I lost Gemini Code Assist when it ended its review activity on 2026-07-17). I cover the details in [CONTRIBUTING.md](../CONTRIBUTING.md#code-review).
...
- [ ] If the PR touches `plugin/` or `.claude-plugin/`, I ran `claude plugin validate --strict` on `./plugin`, `.`, and `./plugin/skills` and saw it pass
- [ ] If the PR touches the Nudge scripts, I confirmed the add → check → ack round trip still works (I also have CI run it on ubuntu + windows)
