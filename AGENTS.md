...
Codex is the sole bot reviewer. Google's consumer Gemini Code Assist app ended all review activity on 2026-07-17, so Gemini no longer reviews PRs here, and I removed its `.gemini/` config. Codex reviews on PR-open and on `@codex review`, and it reaches the operator's review host over Tailscale. A missing Codex review is review-absent, not review-clean: if none appears, enable accept-dns on the review host (`tailscale set --accept-dns=true`), then re-trigger with `@codex review`.
